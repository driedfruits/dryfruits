...
const comparisonSpecs: ComparisonSpec[] = [
  ...
  { 
    key: "organic", label: "Organic Certified", category: "certifications", 
...
export function ProductComparisonTable() {
  ...
  return (
    <div className="bg-card rounded-xl p-6 shadow-soft border border-border/50">
      ...
      {/* Category Filter */}
      <div className="mb-6">
        <label className="text-sm font-medium text-foreground mb-2 block">Filter by Category</label>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {Array.from({ length: maxProducts }).map((_, index) => {
          ...
          return (
            <div key={index} className={`rounded-lg border-2 border-dashed p-3 min-h-[80px] flex items-center justify-center transition-colors ${product ? "border-primary/50 bg-primary/5" : "border-muted-foreground/20 bg-muted/30"}`}>
              {product ? (
                <div className="flex items-center gap-2 w-full">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-foreground truncate">{product.shortName}</p>
                    <p className="text-xs text-muted-foreground truncate">{categoryLabels[product.category] || product.category}</p>
                  </div>
                  <Button variant="ghost" size="icon" className="h-8 w-8 min-h-[48px] min-w-[48px] shrink-0" onClick={() => handleRemoveProduct(product.id)} aria-label={`Remove ${product.name}`}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              ) : (
                <Select value="" onValueChange={handleAddProduct} disabled={availableProducts.length === 0}>
        ...
        <>
          ...
          <div className="hidden md:block overflow-x-auto -mx-6 px-6">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="min-w-[160px] sticky left-0 bg-card z-10">Specification</TableHead>
                  {selectedProducts.map((product) => (
                    <TableHead key={product.id} className="min-w-[140px] text-center">{product.shortName}</TableHead>
                  ))}
                </TableRow>
              </TableHeader>
              <TableBody>
                {Object.entries(specsByCategory).map(([category, specs]) => (
                  <>
                    <TableRow key={`cat-${category}`} className="bg-muted/50">
                      <TableCell colSpan={selectedProducts.length + 1} className="font-semibold text-foreground py-2">{specCategoryLabels[category]}</TableCell>
                    </TableRow>
                    {specs.map((spec) => {
                      const differs = valuesDiffer(spec);
                      return (
                        <TableRow key={spec.key} className={differs ? "bg-amber-50/50 dark:bg-amber-950/20" : ""}>
                          <TableCell className="font-medium text-muted-foreground sticky left-0 bg-inherit">{spec.label}</TableCell>
                          {selectedProducts.map((product) => (
                            <TableCell key={product.id} className="text-center text-sm">{spec.getValue(product)}</TableCell>
                          ))}
                        </TableRow>
                      );
                    })}
                ...
                <TableRow>
                  <TableCell className="font-medium text-muted-foreground sticky left-0 bg-card">Details</TableCell>
                  {selectedProducts.map((product) => (
                    <TableCell key={product.id} className="text-center">
                      <Button asChild variant="outline" size="sm">
                        <Link to={`/products/${product.category}/${product.id}`}>View <ExternalLink className="h-3 w-3 ml-1" /></Link>
                      </Button>
                    </TableCell>
                  ))}
                </TableRow>
          ...
          <div className="md:hidden">
            <div className="flex items-center justify-center gap-2 mb-3">
              {selectedProducts.map((_, idx) => (
                <button key={idx} className="min-h-[48px] min-w-[48px] flex items-center justify-center" onClick={() => carouselApi?.scrollTo(idx)} aria-label={`Go to product ${idx + 1}`}>
                    ...
                    <div className="rounded-lg border border-border bg-background p-4">
                      ...
                      {Object.entries(specsByCategory).map(([category, specs]) => (
                        <div key={category} className="mb-3 last:mb-0">
                          <p className="text-xs font-semibold text-primary uppercase tracking-wide mb-2">{specCategoryLabels[category]}</p>
