...
const EditorPackagingSection = ({ bulk, setBulk, retail, setRetail, custom, setCustom }: EditorPackagingSectionProps) => (
  <div className="space-y-4 rounded-lg border border-border p-4">
    <p className="text-sm font-medium text-foreground">Packaging</p>
    <FormInput label="Bulk" value={bulk} onChange={(e) => setBulk(e.target.value)} placeholder="e.g. 10-12.5kg carton with PE liner" />
    <FormInput label="Retail" value={retail} onChange={(e) => setRetail(e.target.value)} placeholder="e.g. 50g-500g stand-up pouches" />
